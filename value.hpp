...
public:
    ...
    static Value String(const std::string& v) { return Value(ValueType::STRING, v); }
    static Value Bool(bool v) { return Value(ValueType::BOOL, v); }
    static Value List(const std::vector<std::shared_ptr<Value>>& v) { return Value(ValueType::LIST, v); }
    static Value Dict(const std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>& v) { 
    ...
    const std::vector<std::shared_ptr<Value>>& asList() const { return *list_value; }
    const std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>& asDict() const { return *dict_value; }
...
private:
    ...
    union {
        ...
        std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>* dict_value;
    };
    ...
    Value(ValueType t, const std::string& v) : type(t), string_value(new std::string(v)) {}
    Value(ValueType t, const std::vector<std::shared_ptr<Value>>& v) 
        : type(t), list_value(new std::vector<std::shared_ptr<Value>>(v)) {}
    Value(ValueType t, const std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>& v) 
        : type(t), dict_value(new std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>(v)) {}
    ...
    void copyFrom(const Value& other) {
        switch(type) {
            ...
            case ValueType::STRING:
                ...
                break;
            case ValueType::LIST:
                list_value = new std::vector<std::shared_ptr<Value>>(*other.list_value);
                break;
            case ValueType::DICT:
                dict_value = new std::unordered_map<std::shared_ptr<Value>, std::shared_ptr<Value>>(*other.dict_value);
                break;
        }
    }
